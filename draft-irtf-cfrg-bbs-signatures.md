...
8.  for i in (1, ..., U+6):
9.      ell_i = OS2IP(PRF(expand_len)) mod r
10. (r1, r2, e~, r2~, r3~, s~, m~_j1, ..., m~_jU) =
...
16. s' = r2 * r3 + s mod r
17. C1 = A' * e~ + Q_1 * r2~
18. C2 = D * (-r3~) + Q_1 * s~ + H_j1 * m~_j1 + ... + H_jU * m~_jU
19. c_array = (A', Abar, D, C1, C2, R, i1, ..., iR,
                       msg_i1, ..., msg_iR, domain, ph)
20. c_for_hash = encode_for_hash(c_array)
21. if c_for_hash is INVALID, return INVALID
22. c = hash_to_scalar(c_for_hash, 1)
23. e^ = c * e + e~ mod r
24. r2^ = c * r2 + r2~ mod r
25. r3^ = c * r3 + r3~ mod r
26. s^ = c * s' + s~ mod r
...
29. return proof_to_octets(proof)
```
