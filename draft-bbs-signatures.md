...
**Selective Disclosure** - The scheme allows a signer to sign multiple payloads and produce a single output signature. An intermediary (prover) then possessing the payloads and the signature can generate a proof whereby they can choose which payloads to reveal.

**Unlinkable Proof Presentations** - The proofs generated by the scheme are known as a signature proof of knowledge, meaning a verifying party in receipt of a proof is unable to determine which signature was used to generate the proof, removing a common source of correlation. In general each proof generated is indistinguishable from random even for two proofs generated from the same signature.

**Proof of Posession** - The proofs generated by the scheme prove to a verifier that the party who generated the proof (prover) was in posession of a signature without revealing it. Because the scheme also supports binding a presentation header to the generated proof it makes the scheme suitable as a proof of posession protocol. Note the presentation header can include abitrary information such as a cryptographic nonce and or an audience/domain identifier to ensure the generated proof can only be used appropriately, including providing a way for a verifier to detect a replay attack.
