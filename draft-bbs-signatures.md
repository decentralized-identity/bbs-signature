...
This operation generates a secret key (SK) deterministically from a secret octet string (keyMaterial).

For security, IKM MUST be infeasible to guess, e.g. generated by a trusted source of randomness.

keyMaterial MUST be at least 32 bytes long, but it MAY be longer.
...
KeyGen takes an optional input, keyInfo. This parameter MAY be used to derive multiple independent keys from the same key material.

```
SK = KeyGen(keyMaterial, keyInfo)

Inputs:

- keyMaterial (REQUIRED), a secret octet string. See requirements
                          above.
- keyInfo (OPTIONAL), an octet string. Defaults to an empty string if
                      not supplied.
...
1. If length(keyMaterial) < 32, return INVALID
2. km_prime = I2OSP(length(keyMaterial), 8) || keyMaterial
3. info_prime = I2OSP(length(keyInfo), 1) || keyInfo
