...
- okm_key_gen_byte_length, is an integer value representing the number of bytes to read from the XOF when generating the OKM, Note - this value is primarily dependent upon the underlying curve and sub-group being used.

Definitions:

- OS2IP is as defined in [@!RFC8017], Section 4.
...
- okm_key_gen_byte_length: The number of bytes to read from the XOF to be interpreted as the OKM value in key generation
